...
export default function DrawingToolbar() {
  ...
  // Calculate menu width dynamically based on actual rendered content
  useEffect(() => {
    const calculateMenuWidth = () => {
      ...
      // 실제 렌더링된 콘텐츠의 크기를 측정
      // 먼저 fit-content로 설정하여 실제 필요한 크기를 측정
      toolbarRef.current.style.width = 'fit-content';
      toolbarRef.current.style.flexWrap = 'nowrap';
      ...
      requestAnimationFrame(() => {
        ...
        if (contentWidth <= maxScreenWidth) {
          // 1줄에 다 들어감 → 콘텐츠 크기만큼만 사용
          setMenuWidth(`${contentWidth}px`);
          setIsSingleLine(true);
        } else {
          // 1줄에 안 들어감 → 화면 크기까지 넓히고 wrap 허용
    ...
    // 초기 계산 및 약간의 딜레이 (렌더링 완료 후)
    const timer = setTimeout(calculateMenuWidth, 100);
