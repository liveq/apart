...
const FloorPlanCanvas = forwardRef<HTMLDivElement, FloorPlanCanvasProps>(({ measurementMode, calibrationMode, eraserMode, onCalibrationComplete }, ref) => {
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      // Use uploaded image dimensions if available, otherwise use default floor plan
      const imageWidth = uploadedImageDimensions?.width || floorPlan112.width;
      const imageHeight = uploadedImageDimensions?.height || floorPlan112.height;
  ...
  return (
    <div
      ref={ref}
      className="relative w-full h-full overflow-auto bg-muted flex items-center justify-center p-4"
