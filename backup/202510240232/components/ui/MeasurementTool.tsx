...
function MeasurementLine({ measurement, scale, calibratedScale, displayScale, eraserMode, onRemove, t }: MeasurementLineProps) {
  ...
  // Use stored calibratedScale from measurement (not the global one)
  // This allows measurements to maintain their original scale even after recalibration
  const measurementCalibration = measurement.calibratedScale;

  let realDistance = measurement.distance;
  let distanceLabel = '';
  let subLabel = '';

  if (measurementCalibration && displayScale) {
    // Measurement was created with calibration - convert to real distance
    realDistance = measurement.distance * (displayScale / measurementCalibration);
    distanceLabel = `${(realDistance / 10).toFixed(1)} ${t('cm')}`;
    subLabel = `(${(realDistance / 1000).toFixed(2)} ${t('m')})`;
  } else {
    // No calibration - show pixel distance in mm
    distanceLabel = `${measurement.distance.toFixed(0)} mm`;
    subLabel = `(${(measurement.distance / 10).toFixed(1)} cm)`;
  }
