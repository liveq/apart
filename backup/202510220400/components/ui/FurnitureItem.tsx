...
const getResizeCursor = (handle: string, rotation: number): string => {
  ...
  if (finalAngle === 45 || finalAngle === 225) return 'nwse-resize';
  if (finalAngle === 135 || finalAngle === 315) return 'nesw-resize';
...
export default function FurnitureItem({ item, scale, canvasZoom = 1, canvasPanX = 0, canvasPanY = 0, eraserMode = false, drawingEraserActive = false }: FurnitureItemProps) {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // If eraser mode is active (either main toolbar or drawing toolbar), delete the furniture
    if (eraserMode || drawingEraserActive) {
      // Extract timestamp from furniture ID (format: furniture-{timestamp}-{random})
      const idParts = item.id.split('-');
      if (idParts.length >= 2) {
        const createdAt = parseInt(idParts[1], 10);
        const now = Date.now();
        const ageMs = now - createdAt;

        // Don't delete if furniture was created less than 300ms ago
        // This prevents accidental deletion when placing furniture
        if (ageMs < 300) {
          return;
        }
      }
  ...
  return (
    <div
          ...
          return (
            <button
              ...
              style={{
                ...
                transition: 'none',
                // 역회전 없이 translate만 사용 (간단하고 정확)
                transform: isDeleteButtonHovered
                  ? `translate(calc(-50% + ${offsetX}px), calc(-50% + ${offsetY}px)) scale(1.1)`
      ...
      {isSelected && !eraserMode && !drawingEraserActive && (
        <>
          {/* Top edge - full width */}
          <div
