...
export default function DrawingLayer({
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (isDraggingElement && dragStartPoint && dragElementOriginal && selectedElementId) {
      ...
      // Calculate target position based on element type
      let targetX = gridSnapped.x;
      let targetY = gridSnapped.y;

      // For different element types, calculate the reference point
      if (element.type === 'rectangle') {
        targetX = gridSnapped.x - (dragStartPoint.x - element.x);
        targetY = gridSnapped.y - (dragStartPoint.y - element.y);
      } else if (element.type === 'circle') {
        targetX = gridSnapped.x - (dragStartPoint.x - element.cx);
        targetY = gridSnapped.y - (dragStartPoint.y - element.cy);
        ...
        targetX = gridSnapped.x - (dragStartPoint.x - lineCenterX);
        targetY = gridSnapped.y - (dragStartPoint.y - lineCenterY);
      } else if (element.type === 'path' && element.points.length > 0) {
        const firstPoint = element.points[0];
        targetX = gridSnapped.x - (dragStartPoint.x - firstPoint.x);
        targetY = gridSnapped.y - (dragStartPoint.y - firstPoint.y);
      }

      // Detect guides at target position
      const { snappedPos, guides, snaps } = detectGuidesAtPosition({ x: targetX, y: targetY });
      setGuideLines(guides);
      setSnapPoints(snaps);

      // Use snapped position for final update
      const dx = snappedPos.x - dragStartPoint.x;
      const dy = snappedPos.y - dragStartPoint.y;
  ...
  const renderDimensionLabel = (element: DrawingElement) => {
    ...
    if (element.type === 'rectangle') {
      const widthMm = (element.width / canvasWidth) * actualWidth;
      const heightMm = (element.height / canvasHeight) * actualHeight;
      ...
      const widthMm = ((element.rx * 2) / canvasWidth) * actualWidth;
      const heightMm = ((element.ry * 2) / canvasHeight) * actualHeight;
      const widthCm = widthMm / 10;
      const heightCm = heightMm / 10;
      ...
      const lengthMm = (lengthPx / canvasDiagonal) * actualDiagonal;
            ...
            {currentTool === 'rectangle' && (
              <>
                ...
                {(() => {
                  ...
                  const actualWidth = realWidth || (canvasWidth / scale);
                  const actualHeight = realHeight || (canvasHeight / scale);
                  const widthMm = (widthPx / canvasWidth) * actualWidth;
                  const heightMm = (heightPx / canvasHeight) * actualHeight;
                  const widthCm = widthMm / 10;
                  const heightCm = heightMm / 10;
            ...
            {currentTool === 'circle' && (
              <>
                ...
                {(() => {
                  ...
                  const actualWidth = realWidth || (canvasWidth / scale);
                  const actualHeight = realHeight || (canvasHeight / scale);
                  const widthMm = ((rx * 2) / canvasWidth) * actualWidth;
                  const heightMm = ((ry * 2) / canvasHeight) * actualHeight;
                  const widthCm = widthMm / 10;
                  const heightCm = heightMm / 10;
