...
const FloorPlanCanvas = forwardRef<HTMLDivElement, FloorPlanCanvasProps>(({ measurementMode, calibrationMode, eraserMode, onCalibrationComplete }, ref) => {
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (calibrationMode && e.touches.length === 1) {
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    if (calibrationMode && e.touches.length === 1 && calibrationTouchStartPos) {
      e.preventDefault();
      const touch = e.touches[0];
      const container = (ref as React.RefObject<HTMLDivElement>)?.current;
      if (!container) return;
  ...
  }, [calibrationMode, calibrationTouchStartPos, calibrationStart, zoom, panX, panY, canvasWidth, canvasHeight, ref, measurementMode, eraserMode]);

  const handleTouchEnd = (e: React.TouchEvent) => {
    // Calibration mode: handle touch end
    if (calibrationMode && calibrationTouchStartPos) {
  ...
  }, [calibrationMode, calibrationTouchStartPos, calibrationMousePos, isHoldingCalibration, calibrationStart]);
