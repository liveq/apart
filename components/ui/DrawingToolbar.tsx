...
export default function DrawingToolbar() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (lineStylePickerRef.current && !lineStylePickerRef.current.contains(event.target as Node)) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative' }} ref={lineStylePickerRef}>
        <button
          onClick={() => setShowLineStylePicker(!showLineStylePicker)}
          style={{
            padding: '6px 10px',
            backgroundColor: '#f3f4f6',
            border: '1px solid #e5e7eb',
